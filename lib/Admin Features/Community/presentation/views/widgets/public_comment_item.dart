...
class PublicCommentPostItem extends StatefulWidget {
  const PublicCommentPostItem(
    this.gClient, {
    super.key,
    this.onReplay,
  ...
  final CommentsPublicPost commentModel;
  ...
  @override
  State<PublicCommentPostItem> createState() => _PublicCommentPostItemState();
}

class _PublicCommentPostItemState extends State<PublicCommentPostItem> {
  int likeCount = 0;
                ...
                child: CircleAvatar(
                  ...
                  child: CustomNetworkImage(
                    imageUrl: NetworkHandler().getImage(
                      '/Images/Profiles/${widget.commentModel.author!.image}',
                    ),
                    aspectRatio: 1,
                    fit: BoxFit.cover,
                  ),
                    ...
                    IconButton(
                      ...
                      onPressed: () async {
                        if (widget.commentModel.totalLikes != 0) {
                          unLikeComment(
                              widget.post.id!, widget.commentModel.id!);
                          setState(() {});
                        } else {
                          addLikeComment(
                              widget.post.id!, widget.commentModel.id!);
