...
class _ReminderMainBodyState extends State<ReminderMainBody> {
  ...
  List<String> encodedAlerts = alerts.map((alert) => json.encode(alert.toMap())).toList();
...
'All Reminder',
  ...
  Future<void> loadAlerts() async {
    ...
    setState(() {
      alerts = alertsString.map((alertString) => Alert.fromMap(json.decode(alertString))).toList();
    });
  }
    ...
    switch (_selectedTextIndex) {
      ...
      case 0:
       return AllReminder(alerts: alerts,
 ...
 color: kGreenColor,
      ...
      //   children: [
         child:  Padding(
            padding: EdgeInsets.only(top:blockHeight*7, left: blockWidth*6),
            child: SizedBox(
                  ...
                  return GestureDetector(
                    onTap: () {
                     ...
                     child: Row(
                       ...
                       children: [
                         Padding(
                                             padding: EdgeInsets.symmetric(horizontal: blockWidth * .1, vertical: 16),
                         ...
                         SizedBox(width:blockWidth*41 ,),
                          IconButton(
            icon: const Icon(Icons.delete_outline, color: Colors.white, size: 29,),
            onPressed: () {
              deleteSelectedItems();
            },
                          ),
